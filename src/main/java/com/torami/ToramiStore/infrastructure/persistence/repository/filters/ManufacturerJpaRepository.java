...
public interface ManufacturerJpaRepository extends JpaRepository<ManufacturerEntity, Integer> {
    ...
    @Query("SELECT COUNT(f) FROM FigureEntity f WHERE f.manufacturer.id = :manufacturerId")
    int countFiguresByManufacturerId(@Param("manufacturerId") Integer manufacturerId);
